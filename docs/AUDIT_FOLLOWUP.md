...
- ~~**Firestore ルールのフィールド/型/サイズ検証**~~ → **対応済み**。
  `cards` / `boards` に型・長さ・リスト要素数の検証を追加した（1MiB 上限への防波堤）。
  任意フィールドは「存在すれば検証」にして部分更新を壊さない。
  検証は `firestore-rules-tests/` がエミュレータ実測で担保し、CI（`pnpm run test:rules`）で走る。
  **アプリが実際に書く形（最大構成のカード・最小構成・部分更新）が通ることを先に固定**して
  あるので、本番反映で保存できなくなる事故を防げる。
