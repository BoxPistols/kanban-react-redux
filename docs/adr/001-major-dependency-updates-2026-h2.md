...
1. **Firebase 10 → 12**
   ...
   - **セキュリティ動機（監査C9）**: 現行 `firebase@10` の transitive 依存に
     既知の低〜中リスク脆弱性が残存する（`@firebase/auth` 内の `undici < 6.27.0`
     等。主に Node 経路でブラウザバンドルには非搭載だが `pnpm audit` 上は検出される）。
     個別の `pnpm.overrides` で transitive を強制すると SDK の想定バージョンを壊し
     ランタイムを破壊する恐れがあるため、本 Phase の firebase 本体アップグレードで
     一括解消する方針とする（単独の暫定対応は行わない）。
