...
export function ContextMenu({
    ...
    useEffect(() => {
        ...
        // 右クリックされた要素にフォーカスが移ると、ブラウザがその要素を可視域へ運ぶために
        // 数pxスクロールすることがある。これを「スクロールしたから閉じる」と扱うと
        // 開いた直後にメニューが消える。開いた直後の1フレームは scroll を無視する。
        let ready = false
...
// フェードのみ(scale/translate を使わない)。開いた直後の 120ms は要素の矩形が
// 動いてしまい、その間のクリックが「メニュー外クリック」と判定されて閉じる事故になる。
// 位置とサイズを最初のフレームから確定させ、押した場所が必ず狙った項目になるようにする。
